...
def firstpass(pname):
    ...
    def replacements(line):
        ...
        if m:
            line = line.replace(
                "(" + m.group(1), m.group(1) + "  # range was (" + m.group(1)
            )
    ...
    while linenumber < nlines:
        ...
        if len(comments) > 0:
            add(comments[:-1])  # delete final carriage return, which add will attach
        ...
        while True:
            ...
            if m:
                ...
                if name in symbols:
                    ...
                    if (
                        (attr in attrs) and (prim != "curve") and (prim != "display")
                    ):  # curve.x and scene.x are legal
                        newattr = attrs[attr]
                        line = (
                            line[: m.start(1) + start]
                            + name
                            + "."
                            + newattr
                            + line[m.end(2) + start :]
