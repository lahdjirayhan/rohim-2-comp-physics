...
sceneK = display(
    x=0, y=0, width=500, height=500, title="Lagrange Interpolation with Toggle Switch"
)
...
t1 = toggle(
    ...
    width=30,
    height=30,
    text0="1 polynom deg 8",
    text1="4 polynms deg. 2",
    action=lambda: selectpoly(),
)
...
def axis():
    ...
    for i in range(0, 5):  # x tics & labels
        curve(
            pos=[(xmin + xincr * i, ymin), (xmin + xincr * i, ymin - 10)],
            color=color.white,
        )
        xnum = i * 50
        xtext = str(xnum)
    ...
    for i in range(0, 7):  # y tics & labels
        ...
        ytext = str(ynum)
        curve(
            pos=[(xmin - 10, ymin + yincr * i), (xmin, ymin + yincr * i)],
            color=color.white,
        )
