...
ndist = gdisplay(x=500, ymax=200, width=500, height=500, xtitle="Nr", ytitle="N")
...
distribution = ghistogram(
    bins=arange(0.0, Natom, deltaN), accumulate=1, average=1, color=color.red
)
for i in range(0, Natom):  # initial positions and velocities
    col = (1.3 * random.random(), 1.3 * random.random(), 1.3 * random.random())
    x = 2.0 * (L - ar) * random.random() - L + ar  # positons of atoms
    y = 2.0 * (L - ar) * random.random() - L + ar  # in the border forbidden
    ...
    for i in range(0, Natom - 1):
        for j in range(i + 1, Natom):
            ...
            if abs(r2) < ar:  # to avoid 0 denominator
                r2 = ar  #
            invr2 = 1.0 / r2  # compute this factor
