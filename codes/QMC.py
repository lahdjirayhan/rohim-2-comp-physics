...
# Initialize
xscale = 10.0
path = np.zeros([Nsteps], float)
prob = np.zeros([Nsteps], float)

trajec = canvas(width=300, height=500, title="Spacetime Paths")
trplot = curve(
    pos=[(0, y, 0) for y in range(0, 100)], color=color.magenta, canvas=trajec
)


# Axis
def PlotAxes():
    trax = curve(pos=[(-97, -100, 0), (100, -100, 0)], colo=color.cyan, canvas=trajec)
...
def WaveFunctionAxes():
    wvfax = curve(
        pos=[(-600, -155, 0), (800, -155, 0)], canvas=wvgraph, color=color.cyan
    )
    curve(pos=[(0, -150, 0), (0, 400, 0)], canvas=wvgraph, color=color.cyan)
...
def PlotPath(path):
    for j in range(0, N):
        trplot_x_j = 20 * path[j]
        trplot_y_j = 2 * j - 100
        trplot.modify(j, x=trplot_x_j, y=trplot_y_j)
...
def PlotWF(prob):
    for i in range(0, 100):
        ...
        # Center fig
        wvplot_x_i = 8 * i - 400
        wvplot.modify(i, x=wvplot_x_i, y=prob[i])


wvgraph = canvas(x=340, y=150, width=500, height=300, title="Ground State")
wvplot = curve(pos=[(x, 0, 0) for x in range(0, 100)], canvas=wvgraph)
