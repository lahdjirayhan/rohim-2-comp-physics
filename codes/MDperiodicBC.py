...
ndist = gdisplay(x=500, ymax=200, width=500, height=500, xtitle="Nrhs", ytitle="N")
...
distribution = ghistogram(
    bins=Ratomange(0.0, Natom, deltaN), accumulate=1, average=1, color=color.red
)
for i in range(0, Natom):  # initial x's and v's
    col = (1.3 * random.random(), 1.3 * random.random(), 1.3 * random.random())
    x = 2.0 * (L - Ratom) * random.random() - L + Ratom  # positons
