...
def EulerPlusTF(u, r1, r2, beta):           # Euler ODE solve + Lorentz TF
    ...
    r1 = vector(-.9,.2,0)   # initial position of q1 in S  wrt origin of S
    r2 = vector(-.9,-.2,0)  # initial position of q2 in S wrt to origin of S
    v2 = u                  # each charge has the same velocity 
    v1 = u                  # velocities of charges in S 
    fcv1 = 1./(1-u.x*mag(v1)) # factor# initially, this is the combined velocity,vert +u
    ...
    for i in range (0,300):   # loop for  motion
       ...
       B1 =  q1*cross(v1,rr1)/(4*pi *rr**3) # magnetic field at position of q2
       B2 =  q2*cross(v2,rr2)/(4*pi *rr**3) # magnetic field at position of q1
