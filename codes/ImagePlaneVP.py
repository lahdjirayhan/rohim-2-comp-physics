...
scene = canvas(
    width=500, height=500, range=100, title="E of Charge Left of Plane (Red Image)"
)
plane = box(
    pos=vector(0, 0, 0),
    length=2,
...
gridpts = points(size=vector(2, 2, 2), color=color.cyan)
    ...
    for y in range(-60, 80, 20):
        for z in range(-60, 80, 20):
            for x in range(-60, 80, 20):
                ...
                if xx.mag != 0:
                    # E due to q
                    E1 = d / dd**3 if dd != 0 else vector(0, 0, 0)
                    # E due to -q
                    E2 = -dm / ddp**3 if ddp != 0 else vector(0, 0, 0)
                    E = E1 + E2
                    # E
                    elecF = arrow(pos=r, color=color.orange)
                    # 10 x unit vector
                    elecF.axis = 10 * E / mag(E) if mag(E) != 0 else vector(0, 0, 0)
