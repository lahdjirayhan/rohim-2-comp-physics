...
def WL():  # Wang - Landau sampling
    ...
    height = abs(Hsup - Hinf) / 2.0  # Initialize histogram
    ave = (Hsup + Hinf) / 2.0  # about average of histogram
    percent = height / ave
    for i in range(0, L):
        for j in range(0, L):
    ...
    while fac > tol:
        ...
        Enew = (
            Eold
            + 2
            * (sp[ip[xg], yg] + sp[im[xg], yg] + sp[xg, ip[yg]] + sp[xg, im[yg]])
            * sp[xg, yg]
        )  # Change energy
...
for j in range(0, N + 1):
    ...
    if S[j] != 0:
        entrp.plot(pos=(1.0 * order / N, deltaS))  # plot entropy
IntEnergy()
print("Done")
