...
scene2 = canvas( y=400, width=700, height=400, range=500, title="In O', Bz'(Dots) & Ex'"
)
...
def plotB():  
    for i in range(-100, 110, 10):
        for j in range(-50, 60, 10):
            points(pos=vector(i, j, 0), size=4, canvas=scene)
...
def plotBp():  
    for i in range(-500, 501, 8):
        for j in range(-50, 60, 8):
            points(pos=vector(i, j, 0), size=1, canvas=scene2)
