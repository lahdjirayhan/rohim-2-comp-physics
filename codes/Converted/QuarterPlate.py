...
Ey = zeros((xmax, ts), float)
Hx = zeros((xmax, ts), float)
Hxx = zeros((xmax, ts), float)
Hy = zeros((xmax, ts), float)
Hyy = zeros((xmax, ts), float)
Exx = zeros((xmax, ts), float)
Eyy = zeros((xmax, ts), float)

scene = canvas( x=0, y=0, width=800, height=500, title="Ey: \
	cyan, Ex:yellow. periodic BC",
    forward=vector(-0.8, -0.3, -0.7),
)
Exfield = curve(x=list(range(0, xmax)), color=color.yellow, radius=1.5, canvas=scene)
Eyfield = curve(x=list(range(0, xmax)), color=color.cyan, radius=1.5, canvas=scene)
...
plate = box( pos=vector(-100, 0, 0), height=2 * zmax, width=2 * ymax, length=0.5 * xmax, color=vector(1.0, 0.6, 0.0), opacity=0.4, )
