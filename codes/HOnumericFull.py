...
if n % 2 == 0:
    y[0] = 1e-8  # for function parity
else:
    y[0] = -1e-8
y[1] = 0.0  # init cond
f(0.0, y)  # force function at r = 0
dr = 0.01
i = 0

# Compute WF from -5 to +5 in steps of dr
for x in np.arange(-5, 5, dr):
    xx[i] = x
...
plt.plot(xx, yy)
plt.grid()
plt.title("Harmonic Oscillator Wave Function for n = 5")
plt.xlabel("x")
plt.ylabel("Wave Function $\mathbf{\psi(x)}$")
plt.show()
