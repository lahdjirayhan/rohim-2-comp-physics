...
Lbeam = 500
Wbeam = 80
thickness = 10  # beam dimensions
W = 200
WeightBox = 400  # weight of table,box
Lbox = 60
Wbox = 60
Hbox = 60  # Box Dimensions

# Graphics
scene = display(width=750, height=500, range=300)
scene.forward = (0.5, -0.2, -1)  # to change point of view
support1 = cone(pos=(-d, 0, 0), axis=(0, Hsupport, 0), color=color.yellow, radius=20)
support2 = cone(pos=(d, 0, 0), axis=(0, Hsupport, 0), color=color.yellow, radius=20)
beam = box(
    pos=(0, Hsupport + thickness / 2, 0),
    color=color.orange,
    length=Lbeam,
    width=Wbeam,
    height=thickness,
...
cube = box(
    pos=(-d, Hsupport + Hbox / 2 + thickness, 0),
    length=Lbox,
    width=Wbox,
    height=Hbox,
...
Fl = (2 * Wbox + W) / 2.0
arrowFl = arrow(
    color=color.red, pos=(-d, Hsupport + thickness / 2, 0), axis=(0, 0.15 * Fl, 0)
)
Fr = Mg - Fl  # right force
arrowFr = arrow(
    color=color.red, pos=(d, Hsupport + thickness / 2, 0), axis=(0, 0.15 * Fr, 0)
)
...
for t in arange(0.0, 65.0, 0.5):
    ...
    if Fl > 0:
        ...
        x = 300
        beam.rotate(
            angle=-0.2, axis=(0, 0, 1), origin=vector(d, Hsupport + thickness / 2, 0)
        )
