...
def daube4(f, n, sign):
    ...
    if sign >= 0:  # DWT
        ...
        transfgr1 = gdisplay(
            ...
            width=600,
            height=400,
            title="Wavelet Transform, down sampling, Low pass filters",
            xmax=maxx,
            xmin=0,
            ymax=maxy,
            ymin=miny,
        ...
        transfgr2 = gdisplay(
            ...
            width=600,
            height=400,
            title="Wavelet Transform, down sampling, High pass filters",
            xmax=2 * maxx,
            xmin=0,
            ymax=Maxy,
            ymin=Miny,
        )
        transf2 = gvbars(delta=2.0 * n / N, color=color.cyan, display=transfgr2)

        while j <= n - 3:
            rate(speed)

            tr[i] = (
                c0 * f[j] + c1 * f[j + 1] + c2 * f[j + 2] + c3 * f[j + 3]
            )  # low-pass
            transf.plot(pos=(i, tr[i]))  # c coefficients
            tr[i + mp] = (
                c3 * f[j] - c2 * f[j + 1] + c1 * f[j + 2] - c0 * f[j + 3]
            )  # high-pass
        ...
        for i in range(1, mp):
            if i == 1:
                j = 3
            tr[j] = (
                c2 * f[i] + c1 * f[i + mp] + c0 * f[i + 1] + c3 * f[i + mp1]
            )  # low-pass
            j += 1  # upsamplig  c coefficients
            tr[j] = (
                c3 * f[i] - c0 * f[i + mp] + c1 * f[i + 1] - c2 * f[i + mp1]
            )  # high-pass
            j += 1
