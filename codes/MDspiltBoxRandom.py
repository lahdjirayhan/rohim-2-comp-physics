...
ndist = gdisplay(
    ...
    xtitle="Nrhs",
    ytitle="N",
    title="Numb Times Nrhs Particles Occur",
)
distribution = ghistogram(
    bins=arange(1.0, Natoms, deltaN), accumulate=1, average=1, color=color.red
)
scene2 = gdisplay(
    y=400,
    height=400,
...
dis = gvbars(delta=0.5, color=color.red, display=scene2)
...
while t < 1000:
    ...
    for i in range(Natoms):  # Initial x & v
        rate(10)
        x = 2.0 * (L - ar) * random.random() - L + ar  # Atom locations
