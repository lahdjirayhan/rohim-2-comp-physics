...
# time step in S
dt = 0.01
# beta = v/c , v velocity of S' respect  to S, c=1
beta = 0.4
# gamma from special relativity
...
def EulerPlusTF():
    ...
    u = vector(0.4, 0, 0)
    # initial position of q1 in S  wrt origin of S
    r1 = vector(-0.9, 0.2, 0)
    # initial position of q2 in S wrt to origin of S
    r2 = vector(-0.9, -0.2, 0)
    ...
    # transform initial velocities from S to S'
    v1xp = (v1.x - u.x) * fcv1
    v1yp = v1.y * fcv1 / gamma
    v1zp = v1.z * fcv1 / gamma
    v1p = vector(v1xp, v1yp, v1zp)
    v2p = v1p
    # initial positions coincide at t=0 in
    r1p = r1
    # both reference systems S and S'
    r2p = r2
    ...
    for i in range(0, 500):
        # slow the motion
        rate(100)
        # relative position of q2 wrt r1
        rr1 = r2 - r1
        # relative position of q1 relative to r2
        rr2 = -rr1
        # magnitud of vector rr1
        rr = mag(rr1)
        # magnetic field at position of q2
        B1 = vector(q1 * cross(v1, rr1) / (4 * pi * rr**3))
        # magnetic field at position of q1
        B2 = vector(q2 * cross(v2, rr2) / (4 * pi * rr**3))
        # electric field produced by q1 at pos of q2
        E1 = q1 * rr1 / (4 * pi * rr**3)
        # electric field produced by q2 at pos of q1
        E2 = q2 * rr2 / (4 * pi * rr**3)
